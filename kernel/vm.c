...
#ifdef LAB_PGTBL
static pte_t *walksuper(pagetable_t pagetable, uint64 va, int alloc);
static int issuperpage(pagetable_t pagetable, uint64 va);
static int supermappage(pagetable_t pagetable, uint64 va, uint64 pa, int perm);
...
uvmunmap(pagetable_t pagetable, uint64 va, uint64 npages, int do_free)
{
  ...
  while(a < end){
    if(issuperpage(pagetable, a)){
      pte_t *spte = walksuper(pagetable, a, 0);

      if((a % SUPERPGSIZE) == 0 && end - a >= SUPERPGSIZE){
        if(do_free)
          superfree((void*)PTE2PA(*spte));
        *spte = 0;
        a += SUPERPGSIZE;
        continue;
      }
