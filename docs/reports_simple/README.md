...
- 后续每个 Lab 按任务点分节（`==` 二级标题），例如 `sleep (easy)`、`find (moderate)`、`Use superpages (moderate)/(hard)`。
- 每个 Lab 开头用「要解决什么问题 → 出于什么动机 → 有怎样的初步想法」做引入，再进入正文；用 `#part("前置知识")` 把依赖的基础概念讲透，并给出查阅路径（xv6 教材章节、RISC-V 规范、搜索关键词、关键源文件）。
- 任务点内部不再套「实验目的/实验步骤/问题/心得」四段固定小节，而是按知识点的自然学习顺序写：流程只梳理不铺满，挑 1–2 个核心知识点深入，用第一人称写真实理解和踩坑。
- 代码仓库链接需要在正文中给出：`https://github.com/Jambity11/xv6-labs-2025`。
