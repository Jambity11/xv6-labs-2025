...
本仓库为 MIT 6.1810 / xv6-labs-2025 课程实验的个人实现与实验报告整理，包含 xv6 内核源码修改、各实验测试结果以及实验报告材料。
...
不同实验的完整评分通常需要切换到对应实验分支或调整 `conf/lab.mk`。调试阶段建议先运行对应的单项测试程序，确认单项通过后再运行 `make grade`。
